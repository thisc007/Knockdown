        ...
        <div class="panel panel-info">
            <div class="panel-heading">
                <h2 style='text-transform:uppercase'><?php
                    ...
                    echo $user->lastname;
                    ?>&nbsp;&nbsp;<?php
                    echo $this->Form->button('<i class="fa fa-edit"></i> Editar', ['class' => 'btn btn-info  btn-social  ', 'escape' => false, 'onclick' => "location.href='" . $this->Url->build(['action' => 'edit2', $user->id]) . "'"]);
                ...
                <div class="row" >
                    <div class="col-lg-2">
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <?php
                                echo $this->Form->button('<i class="fa fa-camera-retro"></i> Alterar Foto', ['class' => 'btn btn-link', 'escape' => false, 'onclick' => "location.href='" . $this->Url->build(['controller' => 'files', 'action' => 'addpic', '?' => ['ctrl' => $this->request->params['controller'], 'ctrlid' => $user->id]]) . "'"]);
                                ?>
                            </div>                    
                        </div>
                    ...
                    <div class="col-lg-10">
                        ...
                        <p><strong><?= __('Data de Nascimento: ') ?></strong><?= $user->borndate ? $user->borndate->day . '/' . $user->borndate->month . '/' . $user->borndate->year : '' ?></p>
                        ...
                        <p><strong><?= __('Data de matrícula: ') ?></strong><?= $user->enrollment_date ? $user->enrollment_date->day . '/' . $user->enrollment_date->month . '/' . $user->enrollment_date->year : '' ?></p>
...
<div class="row">
    <div class="col-lg-12">
        ...
        <h5>
            <?php
            echo $this->Form->button('<i class="fa fa-plus-circle "></i> Adicionar Telefone', ['class' => 'btn btn-primary  btn-social', 'escape' => false, 'onclick' => "location.href='" . $this->Url->build(['controller' => 'phones', 'action' => 'add', '?' => ['controller' => 'users', 'controllerid' => $user->id, 'action' => 'aboutme']]) . "'"]);
            ?>
        </h5>
                ...
                <?php foreach ($phones as $phone): ?>
                    <tr>
                        ...
                        <td class="actions">
                            ...
                            <?php
                            echo $this->Form->button('<i class="fa fa-edit"></i> ', ['title' => 'Editar', 'class' => 'btn btn-warning  btn-social-icon btn-lg', 'escape' => false, 'onclick' => "location.href='" . $this->Url->build(['controller' => 'phones', 'action' => 'edit', $phone->id, '?' => ['controller' => 'users', 'controllerid' => $user->id, 'action' => 'aboutme']]) . "'"]);

                            //$this->Html->link(__('Editar'), ['controller'=>'Companiesphones','action' => 'edit', $phone->id]) 
                            ?>&nbsp;
                            <?php
                            echo $this->Form->button('<i class="fa fa-minus-circle"></i> ', ['title' => 'Excluir', 'class' => 'btn btn-danger btn-social-icon btn-lg', 'escape' => false, 'onclick' => "var r = confirm('" . __('Você quer apagar o telefone?') . "');if (r == true) {location.href='" . $this->Url->build(['controller' => 'phones', 'action' => 'delete', $phone->id, '?' => ['controller' => 'users', 'controllerid' => $user->id, 'action' => 'aboutme']]) . "';}else{return false;}"]);
...
<div class="row">
    <div class="col-lg-12">
        ...
        <h5>
            ...
            <?php
            echo $this->Form->button('<i class="fa fa-plus-circle"></i> Adicionar Endereço', ['class' => 'btn btn-primary  btn-social  btn-social', 'escape' => false, 'onclick' => "location.href='" . $this->Url->build(['controller' => 'addresses', 'action' => 'add', '?' => ['controller' => 'users', 'controllerid' => $user->id, 'action' => 'aboutme']]) . "'"]);
            ?>
                ...
                <tbody>
                    <?php foreach ($addresses as $address): ?>
                        <tr>
                            ...
                            <td class="actions">
                                <?php
                                echo $this->Form->button('<i class="fa fa-edit"></i> ', ['title' => 'Editar', 'class' => 'btn btn-warning  btn-social-icon btn-lg', 'escape' => false, 'onclick' => "location.href='" . $this->Url->build(['controller' => 'addresses', 'action' => 'edit', $address->id, '?' => ['controller' => 'users', 'controllerid' => $user->id, 'action' => 'view']]) . "'"]);

                                //$this->Html->link(__('Editar'), ['controller'=>'Companiesaddresses','action' => 'edit', $address->id]) 
                                ?>&nbsp;
                                <?php
                                echo $this->Form->button('<i class="fa fa-minus-circle"></i> ', ['title' => 'Excluir', 'class' => 'btn btn-danger btn-social-icon btn-lg', 'escape' => false, 'onclick' => "var r = confirm('" . __('Você quer apagar o endereço?') . "');if (r == true) {location.href='" . $this->Url->build(['controller' => 'addresses', 'action' => 'delete', $address->id, '?' => ['controller' => 'users', 'controllerid' => $user->id, 'action' => 'aboutme']]) . "';}else{return false;}"]);
